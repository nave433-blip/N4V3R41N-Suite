...
CHIP_MAP = {
    ...
    "iPhone9,1": "A10", "iPhone9,3": "A10", "iPhone10,1": "A11", "iPhone10,4": "A11",
...
[/bold cyan]
[bold dim]      >> THE ULTIMATE UNIFIED BYPASS SUITE v5.0 <<[/bold dim]
"""
...
class Dashboard:
    ...
    def __rich__(self) -> Panel:
        ...
        if self.device:
            table.add_row(Text("STATUS:", style="bold magenta"), Text("ONLINE", style="bold green"))
            ...
            table.add_row(Text("UDID:", style="bold magenta"), Text(self.device['udid'][:12] + "...", style="dim"))
        else:
            table.add_row(Text("STATUS:", style="bold magenta"), Text("WAITING FOR DEVICE...", style="bold blink red"))
        
        return Panel(Align.center(table), title="[bold cyan]CORE DIAGNOSTICS[/bold cyan]", border_style="cyan")
...
def main_menu():
    ...
    options = [
        ...
        ("5", "[bold magenta]PURPLE MODE[/bold magenta]", "Enter A10-A11 Diagnostic Mode"),
        ("6", "[bold cyan]JAILBREAK SUITE[/bold cyan]", "Run Dopamine, Palera1n, or Xina"),
        ("7", "[bold dim]C2/GESALT SERVERS[/bold dim]", "Start local activation servers"),
        ("8", "[bold white]INTEGRATED TOOLS[/bold white]", "Metasploit, Tr4mpass, iRecovery"),
        ("9", "[bold red]N4V3R41N DOCTOR[/bold red]", "System health & dependency fix"),
...
def full_auto_mode(device):
    if not device:
        console.print("[red][!] DEVICE NOT DETECTED.[/red]")
        return
    
    chip = device.get("chip", "unknown")
    console.print(f"[blue][*] AUTO-DETECTED TARGET: {chip}[/blue]")
    
    if chip in ["A12", "A13", "A14", "A15"]:
        run_a12_bypass()
    else:
        console.print("[yellow][!] DEFAULTING TO PATH-A EXPLOIT...[/yellow]")
        run_command(["./exploits/tr4mpass/tr4mpass"])
