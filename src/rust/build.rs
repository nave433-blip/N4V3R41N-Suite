...
fn main() {
    ...
    let c_files = [
        ...
        "bypass/path_b.c",
        "bypass/path_c_sparse.c",
        "bypass/repair.c",
        "bypass/signal.c",
        "bypass/sparse_restore.c",
        "bypass/purple_mode.c",
        "device/chip_db.c",
        "device/device.c",
        "device/usb_dfu.c",
        "util/env_config.c",
    ...
    build_cpp.cpp(true);
    build_cpp.flag(&format!("-mmacosx-version-min={}", deployment_target));
    build_cpp.include(base_c_path);
    build_cpp.include(format!("{}/bypass", base_c_path));
    build_cpp.include(format!("{}/device", base_c_path));
    build_cpp.include(format!("{}/util", base_c_path));
    build_cpp.include("/usr/local/include");
    
    build_cpp.file(format!("{}/bypass/new_features.cpp", base_c_path));
    build_cpp.compile("libn4v3r41n_core_cpp.a");
